...
vectorMatrixMultCompareRow( uint32_t *A, uint32_t *B, uint32_t *C, 
    ...
    for (int i = 0; i <= ((width - 1) / blockDim.x); i++) {
        if ((i * blockDim.x + threadIdx.x) < width) {
            ...
            cTruthEntry = (tex2D(texRef, i * blockDim.x + threadIdx.x, rowToBeChanged));
...
vectorMatrixMultCompareCol(uint32_t *A, uint32_t *B, uint32_t *C, 
    ...
    for (int i = 0; i <= ((height - 1) / blockDim.x); i++) {
        if ((i * blockDim.x + threadIdx.x) < height) {
            ...
            //cTruthEntry = (C[intId] >> 32 - intLane - 1) & 1; 
            cTruthEntry = (tex2D(texRef, colToBeChanged, i * blockDim.x + threadIdx.x));	
...
void initializeTextureMemory(uint32_t **C0, int width, int height) {
    ...
    CUERR
    cudaMemcpyToArray(cuArray, 0, 0, *C0, sizeof(uint32_t) * width * height, cudaMemcpyHostToDevice);
    CUERR
    
    texRef.addressMode[0] = cudaAddressModeWrap; CUERR
    texRef.addressMode[1] = cudaAddressModeWrap; CUERR
    cudaBindTextureToArray(texRef, cuArray, channelDesc); CUERR
